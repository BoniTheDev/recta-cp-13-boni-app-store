...
class AppStore extends Component {
  ...
  render() {
    ...
    const getFilterAppsByTabId = this.getFilterProjectByTabid()

    const filterBySearch = getFilterAppsByTabId.filter(filterApp =>
      filterApp.appName.toLowerCase().includes(searchInput.toLowerCase()),
    )
        ...
        <ul className="tab-items-list">
          {tabsList.map(tabDetails => (
            <TabItem
              ...
              onUpdateActiveTabId={this.updateActiveId}
              isActiveTab={tabDetails.tabId === activeTabId}
            />
          ))}
        </ul>
        <ul className="app-items-list">
          {filterBySearch.map(appDetails => (
